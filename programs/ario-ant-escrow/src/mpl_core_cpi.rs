...
// =========================================
// UpdateV1 — REMOVED (ADR-028)
// =========================================
//
// Escrow no longer touches the asset UpdateAuthority. Program-controlled ANTs
// keep UA permanently at the ario-ant `ant_authority` PDA, so deposit / claim /
// cancel only move Owner (TransferV1) — which is Owner-gated and unaffected by
// where UA lives. This structurally resolves the original audit-L23 concern
// (a depositor retaining UA to rewrite metadata post-claim): the depositor
// never holds UA at any point. The former `encode_update_v1_set_ua_ix_data` /
// `update_v1_metas` / `set_update_authority_signed_by_{wallet,pda}` helpers and
// their tests were deleted along with this behavior.
