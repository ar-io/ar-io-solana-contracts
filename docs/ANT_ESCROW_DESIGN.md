...
Vaults are claimable **at any time** (ADR-027). Settlement branches on the remaining lock (`remaining = end_timestamp - now`):

- `remaining >= ArioConfig.min_vault_duration` (read live from ario-core at claim time): **re-lock** — the handler does an atomic payer pass-through (escrow → `payer_token_account` for exactly `amount`, escrow-PDA-signed) and then, in the same instruction, CPIs `ario_core::vaulted_transfer(amount, remaining, revocable=false)` with `sender = payer`, `recipient = claimant` (or `ario_core::create_vault` when `payer == claimant`, since `vaulted_transfer` rejects sender == recipient). The claimant receives a native, non-revocable ario-core vault unlocking at the escrow's exact original `end_timestamp`. The re-lock account set rides as six trailing `Option<>` accounts on both claim instructions; a still-locked claim without them fails `RelockAccountsMissing`.
- `remaining < min_vault_duration` (including expired): **liquid** SPL transfer to the claimant. This means a vault is liquid-claimable up to `min_vault_duration` (default 14 days) early — a deliberate, bounded consequence (ADR-027): ario-core would reject a shorter re-lock, and forcing near-expiry claimants to come back defeats the escrow's migration-window purpose.

> **History:** the original active path (ADR-014 era) used *transaction introspection* of a sibling `vaulted_transfer`, which had no 1:1 binding between a claim and the re-lock it credited — one sibling could satisfy multiple batched claims (lock bypass / relayer skim; Codex finding), so **ADR-022 (2026-05-28) removed it** (`VaultStillLocked` on active claims, liquid-at-expiry only). **ADR-027 (2026-07-01) restored** the capability via the direct CPI described above — 1:1 by construction and atomic (any CPI failure reverts the pass-through), against ario-core's existing, unmodified ABI. See **ADR-022** (its introspection analysis remains authoritative) / **ADR-027** / **BD-113**.

The revocable-controller theft variant on the original active path was independently closed by **ADR-021** / **BD-105**, and `deposit_vault` still rejects `revocable=true` (`RevocableVaultUnsupported`); the ADR-027 re-lock CPI hardwires `revocable = false`. `ario_core` revocable vaults are unaffected for direct use; only the escrow declines to produce/accept them.
