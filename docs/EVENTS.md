...
> every state-changing instruction (90 events, 127+ emit sites). Consume
...
| `ario-gar` | 36 | Gateway lifecycle, stake (operator/delegate/redelegate), withdrawals, epoch lifecycle (create→tally→prescribe→distribute→close), multi-source funding plan, admin-authority transfer (ADR-026), EpochSettings authority transfer (ADR-0031) |
