...
| **Solana Behavior** | `request_and_set_primary_name` and `approve_primary_name` accept either the ANT NFT holder (existing path) **or** the `AntRecord.owner` for that undername, when the requested name is an undername (contains `_`). Base names still require ANT-holder auth. The fallback branch reads `remaining_accounts[3]` (request_and_set) / `remaining_accounts[2]` (approve) as the `AntRecord` PDA, validates owner program + PDA derivation + discriminator + Borsh layout, and requires `Some(owner) == initiator`. Lazy reconciliation in ario-ant clears `AntRecord.owner` whenever the NFT is observed to have transferred, so a stale record owner can never override a new ANT holder. **(2026-06-02, BD-109: the canonical `owner = None` root-record path no longer trusts the cached `last_reconciled_owner` — it reads the live Metaplex Core holder from the asset account, which all four handlers now require. The base-name/`@` auth is now live-owner, not snapshot.)** |
...
### BD-109: Primary-Name Authorization Reads the Live ANT Holder for owner=None Records (2026-06-02)

| | |
|---|---|
| **Lua Behavior** | Only the ANT process owner could create/approve/remove a primary name (per BD-092/BD-097). Transferring the ANT moves that authority. |
| **Regression** | ario-core's `read_ant_record_owner` resolved an `AntRecord` with `owner = None` (the canonical `@` root-record state) to the record's cached `last_reconciled_owner`. That per-record cache is written at spawn and only refreshed lazily by a later `ario_ant` record op; the wrapped `ario_ant::transfer` updates `AntConfig.last_known_owner` but **not** each record's LRO, and direct Metaplex transfers update neither. So after selling an ANT, the prior holder's key still matched the stale LRO and could `request_and_set` / `approve` / `remove` primary-name + reverse records for a base name they no longer controlled, until someone ran a record op on that exact record. Introduced by the `owner.unwrap_or(last_reconciled_owner)` fallback (PR #34). Medium-severity stale-authorization; found by a Codex security review, 2026-06. Same class as BD-106. |
| **Solana Behavior (fixed)** | For `owner = None` records, `read_ant_record_owner` now reads the **current** Metaplex Core NFT holder LIVE from the asset account instead of the cached LRO. Each of the four primary-name handlers (`request_and_set_primary_name`, `approve_primary_name`, `remove_primary_name_for_base_name`, and the funding-plan `request_primary_name_from_funding_plan` variant) appends the ANT MPL asset account to its account list; the helper validates `asset.key() == ant_mint` and the asset's program-`owner == MPL_CORE_PROGRAM_ID`, then reads the owner bytes via a local `read_mpl_core_owner` (a copy of `ario_ant` / `ario_arns`'s helper — no `mpl-core` crate, no CPI, ADR-016-compatible) and requires `owner == signer`, else `ArioError::NotAntHolder`. Explicit-delegate records (`owner = Some(x)`) are unchanged (authorize against `x`). `last_reconciled_owner` is no longer read by ario-core. |
| **Integrator impact** | The four handlers now take an additional ANT MPL asset account: `request_and_set_primary_name` → `remaining_accounts[3]`; `approve_primary_name` / `remove_primary_name_for_base_name` → `remaining_accounts[2]`; `request_primary_name_from_funding_plan` → `validation_accounts[3]` (and `validation_account_count` incremented by 1). The asset is the current `record.ant` (== the ArNS record's ANT mint), read-only. SDK / tooling that builds these instructions must pass it — see the `ar-io-sdk` / `solana-ar-io` follow-up. |
| **Why** | Parity with Lua and the whitepaper model where holding the ANT NFT confers naming authority; mirrors the BD-106 fix for ario-arns reassign/release. The bug conflated a per-record cache with live ownership. |
...
| Primary Name Authorization | 1 (BD-097) |
