...
| **Solana Behavior** | `MAX_UNDERNAME_LIMIT = 10_000` (in `ario-arns/src/state/mod.rs`). Every `increase_undername_limit*` handler rejects increases that would push the total above this cap, via `ArnsRecord::checked_undername_limit`. |
| **Rationale** | 10,000 is the **v3.0.0 whitepaper's** number (§9.2, in the same table as the default limit of 10 — see `docs/WHITEPAPER_COMPARISON.md`), so enforcing it is whitepaper parity, not a port invention; the whitepaper is canonical where it and Lua disagree (as with the primary-name fee). It also prevents unbounded undername records per ANT, which would make enumeration and account management impractical. Nothing on chain is sized by the limit — it is a counter, and each undername is its own `AntRecord`/`AntRecordMeta` PDA pair with its own rent and per-undername fee — so the cap is a policy ceiling, not a technical one. `undername_limit` is `u16`, so raising it up to 65,535 is a one-constant, non-breaking relaxation needing no migration; enforcing a *lower* number later would be breaking, which is why the documented ceiling is the one to enforce now. Any change to the number is a whitepaper/spec decision, not a contract one. |
| **Enforcement gap, fixed 2026-09-17** | This entry described the cap from the start, but **no such constant existed** and only the two stake-funded handlers enforced the number, as a bare `10_000` literal. `increase_undername_limit` — the handler this entry names — plus the delegation and operator-stake variants had no cap at all: they did a `u16` `checked_add`, so a name could be driven to `u16::MAX` (65,535), 6.5× the documented ceiling, and the increase is permissionless (anyone may pay to raise someone else's limit, `arns.lua:278`). Nothing asserted the cap on any path. Now: the constant exists, all five handlers go through `ArnsRecord::checked_undername_limit`, and the cap and its `u16` overflow edge are unit-tested plus asserted end-to-end by `test_increase_undername_limit_respects_the_cap`. A dedicated error (`UndernameLimitExceeded`, appended per ADR-035) replaces the previous `InvalidParameter`. No mainnet or staging record exceeds the cap — the largest is 9,999, the AO-imported name `1.23454E+33` — so the fix is preventive. That one record can add exactly one more undername before the ceiling binds; it is the only live argument for raising the number, and doing so is a separate one-constant change (see the rationale row). |
| **Grace-period divergence, fixed 2026-09-17** | Lua's `assertValidIncreaseUndername` requires an **active** record (`arns.lua:938`; `recordIsActive` is `endTimestamp >= now`), and the balance, delegation and operator-stake paths matched it. The `_from_withdrawal` and `_from_funding_plan` variants also accepted a record inside its grace period, so whether an expired name could still buy undernames depended on how the buyer paid. Aligned to Lua and to the other three paths, and asserted in `test_extend_and_increase_from_withdrawal_leave_purchase_price`. Extending a lease still works in the grace period on every path, which *is* Lua-faithful (`assertValidExtendLease` refuses only a fully expired record). |
...
| **Wave 2 amendment (2026-09-18)** | Three events appended to `ario-gar` — all additive, no shipped event changed shape (ADR-018). (1) `EpochSkippedNoObservationsEvent { epoch_index, active_gateway_count, timestamp }` (ADR-0034 addendum): the epoch collected zero observations, so `distribute_epoch` paid nothing and credited no gateway stats. **It is emitted immediately BEFORE an otherwise-normal `EpochDistributedEvent` carrying `gateways_processed: 0, total_eligible_rewards: 0`.** Consumers must not infer a skip from those zero totals — a normal distribution with an empty eligible set reports identically; `EpochDistributedEvent`'s frozen shape cannot carry a flag, so the new event is the only stable discriminator. Anything that only needs "this epoch finished" keeps watching `EpochDistributedEvent` and needs no upgrade. (2) `DelegatedStakeReconciledEvent { gateway, previous, removed, new, delegations_counted, timestamp }` (ADR-0037): a gateway's `total_delegated_stake` was lowered to the sum of the Delegation accounts proven to back it, removing phantom stake left by the AO import; `delegations_counted` lets an indexer tell a genuinely zero-delegation gateway from one whose delegations were omitted. (3) `SupplyCountersResyncedEvent { previous_staked, new_staked, previous_delegated, new_delegated, timestamp }` (ADR-0037): the reporting-only `GatewaySettings` supply counters were set to audited values. Full shapes and consumer guidance in [`EVENTS.md`](./EVENTS.md#wave-2-additions-adr-0034--adr-0037). |
...
| **Excess-lock correction (2026-05-18)** — ⚠️ SUPERSEDED: its premise (Lua = 30 days) is wrong; the 30-day excess lock is a deliberate divergence, see the 2026-09-17 row | Original BD-102 (2026-05-04) incorrectly described `withdrawLengthMs` as 90 days. The Lua source has it as 30 days. The Solana port matched the (incorrect) doc and locked the excess vault for 90 days too. Corrected: `leave_network` now uses `settings.withdrawal_period` for the excess vault's `available_at`, restoring Lua parity. Same fix in `claim_delegate_from_leaving_gateway` (was using the hardcoded `WITHDRAWAL_LOCK_PERIOD` const; now reads from settings so the admin lever applies). |
| **Prune excess-lock fix + Lua re-reading (2026-09-17)** | Two things. (1) **The 2026-05-18 correction above misread Lua.** `constants.lua` has set `operators.withdrawLengthMs = daysToMs(90)` since 2025-02-07 (`d111500`), the same as `leaveLengthMs`; Lua locks BOTH vaults for 90 days. Solana's 30-day excess lock is therefore a deliberate divergence, consistent with the port shortening the regular withdrawal lock to 30 days (see the constants table below), **not** Lua parity. (2) **`prune_gateway` was never updated by that change** and kept giving its excess vault the 90-day leave lock, so a pruned operator's above-minimum stake was locked three times longer than a voluntary leaver's — with nothing in the code, tests or docs saying why. Fixed: `prune_gateway` now uses `settings.withdrawal_period` for the excess vault, matching `leave_network`. Both vaults' periods on both paths are now pinned by `test_leave_network_vault_lock_periods`, `test_prune_gateway_vault_lock_periods` and `test_prune_gateway_excess_follows_withdrawal_period_setting`. **Mainnet impact:** 3 pruned operators had a 90-day excess vault (`2MKD2JLJ…` 36,177.88 ARIO, `Cn79LdBT…` 57,212.67, `FrRJpBnFbg…` 878,549.93; created 2026-08-04/14/05). Their vaults keep the stored 90-day `available_at` — the fix is not retroactive — but they are `is_protected: false`, so `instant_withdrawal` is open to them at the minimum 10% penalty (the penalty decays over `settings.withdrawal_period`, which has fully elapsed for all three). The protected 90-day vault is unaffected and intended. The resulting rule — and what was reviewed and deliberately left unchanged (the 100% slash, the protected vault's 90-day lock, prune's second protected slice) — is [ADR-0038](adrs/0038-operator-exit-unlock-schedule.md). |
...
### BD-112: Demand Factor Has a Maximum (CU-Safety Ceiling) (2026-06-03)

| | |
|---|---|
| **Lua Behavior** | `demand.lua` clamps the demand factor only at the **floor** (`math.max(demandFactor, demandFactorMin)`); the 1.05x up-adjustment (`getDemandFactor() * (1 + upAdjustmentRate)`) has **no ceiling**, so under sustained increasing demand the factor (and thus fees) can grow without bound. AO has no per-message compute budget, so an unbounded factor is harmless there. |
| **Solana Behavior** | `current_demand_factor` is clamped at **`MAX_DEMAND_FACTOR = 1000 × DEMAND_FACTOR_SCALE`** (1000.0) in `roll_one_period`'s up-path. The floor and all other dynamics are unchanged. |
| **Rationale** | The lazy-rollover (`maybe_roll_demand_period`) fast-forwards a long-idle gap (>100 periods) in **closed form** to stay within the pricing-tx CU budget while preserving every zero-activity decay and permanent fee-halving (fixing the destructive-skip bug — Codex "rollover cap permanently skips demand-fee decay"). The closed form's only unbounded term was the initial decay from the persisted factor back to the floor — `~log₀.₉₈₅(MIN / factor)` steps. Without a ceiling, a hot name's accumulated factor could make that decay arbitrarily long, reintroducing a state-dependent compute cliff (CodeRabbit, PR #97). A factor ceiling bounds that decay to **≤ ~510 steps**, making the whole rollover compute-bounded regardless of gap size or accumulated demand. |
| **Solana-only nuance** | `MAX_DEMAND_FACTOR` is set to **1000x** — far above any economically reachable value (reaching it requires ~141 consecutive periods of geometrically-rising demand, i.e. fees at 1000x base). It is a compute-safety bound, not a pricing-policy change: realistic demand never approaches it, so observable pricing is unchanged vs. the uncapped Lua model. If a future deployment wanted the cap to be pricing-relevant, lowering it is a separate economic decision. No account-layout change (a new `const`, not a stored field). |

### BD-113: Escrow Active-Vault Claims Re-lock via Direct CPI; Sub-Minimum Remainders Deliver Liquid Early (2026-07-01)

| | |
|---|---|
| **Lua Behavior** | AO has no escrow; migrated vaults retain their original lock and unlock naturally. There is no "claim a still-locked vault early" concept. |
| **Solana Behavior (before, BD-107)** | Between ADR-022 and ADR-027, a still-locked vault claim was rejected with `VaultStillLocked`; vaults were claimable liquid only after `vault_end_timestamp`. |
| **Solana Behavior (now)** | Every vault escrow is claimable at any moment. `claim_vault_arweave_attested` / `claim_vault_ethereum` branch on `remaining = vault_end_timestamp - now`: (1) `remaining >= ArioConfig.min_vault_duration` (read live from ario-core at claim time) → **re-lock**: an atomic payer pass-through (escrow → `payer_token_account` for exactly `amount`, escrow-PDA-signed) followed by a same-instruction CPI into ario-core's existing `vaulted_transfer` (or `create_vault` when `payer == claimant`, since `vaulted_transfer` rejects sender == recipient), creating a claimant-owned, **non-revocable** vault unlocking at the escrow's exact original `vault_end_timestamp`; (2) `remaining < min_vault_duration`, including expired → **liquid** transfer to the claimant. The re-lock account set rides as six trailing `Option<>` accounts (omitted on expired claims — the ADR-022-era claim ABI keeps working; required for ANY still-locked claim, even one that settles liquid via the fallback); a still-locked claim without them fails `RelockAccountsMissing`. |
| **Why** | The escrow program is temporary — forcing claimants to return after expiry is undesirable. ADR-022's reuse/redirection hole was structural to *introspection* (no 1:1 claim↔re-lock binding); the direct CPI is 1:1 by construction and atomic, so restoring the feature does not reopen the finding. ario-core is untouched (hard constraint): the payer pass-through exists precisely because `vaulted_transfer` fuses rent-payer and token-sender into one signer, which the escrow PDA cannot be. See ADR-027. |
| **Deliberate consequence** | A vault escrow becomes liquid-claimable up to `min_vault_duration` (default 14 days) **before** its nominal end — the depositor's effective lock is `vault_end_timestamp - min_vault_duration`. Accepted intentionally: ario-core would reject a shorter re-lock (`LockDurationTooShort`), and both alternatives (reject → forced return visit; round up → silent over-lock) are worse for a migration-window program. Claiming is never worse than waiting. `VaultStillLocked` is now unreachable but retained (append-only error ABI). |
...
### BD-117: `purchase_price` Records the Name Purchase, Not Later Spend (2026-09-17)

| | |
|---|---|
| **Lua Behavior** | `arns.lua` writes `purchasePrice` in exactly two places: `buyRecord` sets it to the registration fee when the name is bought (`arns.lua:116`), and `upgradeRecord` overwrites it with the permabuy fee (`arns.lua:879`). `extendLease` and `increaseUndernameLimit` never touch it, so it always answers "what was paid for this name", never "what has been spent on it in total". |
| **Solana Behavior (before)** | The five funding modes of each fee-paying instruction are meant to differ only in where the money comes from. They did not: `extend_lease` and `increase_undername_limit` left `purchase_price` alone on the balance, delegation and operator-stake paths (Lua-faithful), but their `_from_withdrawal` and `_from_funding_plan` variants **added** the fee into it. The same action therefore produced different stored state depending on how it was paid, on 4 of the 25 handlers. |
| **Solana Behavior (now)** | Those four writes are removed. `purchase_price` is written only by `buy_name`, `buy_returned_name` and `upgrade_name`, on all five funding modes each, matching Lua. |
| **Blast radius** | Reporting only. No on-chain logic reads `purchase_price`: the returned-name premium is recomputed from the current base fee and demand factor (`calculate_returned_name_premium`), and `ReturnedName` does not store the old price. The field is surfaced by the SDK (`getArNSRecord().purchasePrice`) and the Network Portal, so an affected record over-reported what its name cost. Records extended or given undernames through a withdrawal vault or a multi-source funding plan before this fix keep the inflated value; nothing recomputes it. |
| **How it was found** | A sweep for the shape behind [ADR-0038](adrs/0038-operator-exit-unlock-schedule.md): sibling instructions that must produce identical state, where nothing asserts that they do. The same sweep confirmed the other dimensions are uniform — demand-factor rolling, `purchases_this_period` / `revenue_this_period`, prune-timestamp scheduling and every other record field agree across all 25 handlers; every GAR unlock timestamp reads `settings.withdrawal_period` except the two protected exit vaults; and the only supply-counter gap is `compound_delegation_rewards` (ADR-0037). |
| **Now asserted by** | `test_extend_and_increase_from_withdrawal_leave_purchase_price`, plus assertions added to `test_extend_lease_from_funding_plan_multi_gateway` and `test_increase_undername_limit_from_funding_plan_multi_gateway`. Nothing asserted this on any path before. |
...
### BD-118: An Unfinished Epoch Cannot Be Superseded, and an Unobserved One Pays Nothing (2026-09-18)

| | |
|---|---|
| **Lua Behavior** | Epoch advancement is internal and strictly sequential: the AO tick distributes epoch N and only then moves on, so "create N+1 while N is unfinished" is not representable and needs no guard. For payouts, `epochs.lua:636` sets `totalObservationsSubmitted` from the submitted reports and `:651` computes `failed = #observersMarkedFailed > (totalObservationsSubmitted / 2)`. With **zero** observations that is `0 > 0` → false, so **every** gateway is treated as having passed: `:659-663` increment `totalEpochCount`, `passedEpochCount` and `passedConsecutiveEpochs`, and reset `failedConsecutiveEpochs` to 0. An unobserved epoch in Lua therefore pays everyone and credits everyone a pass. (`ar-io-network-process` @ `2e52958`.) |
| **Solana Behavior (before)** | `create_epoch` and `tally_weights` were both permissionless with no gate on the previous epoch's state, and `distribute_epoch` reproduced Lua's rule exactly — including crediting a pass to every gateway when no observations were submitted. |
| **Solana Behavior (now, ADR-0034)** | `create_epoch` requires the previous Epoch PDA in `remaining_accounts` and refuses with `LatestEpochUnfinished` (6102) unless it is distributed or absent (written off); a missing account is refused separately with `MissingLatestEpochAccount` (6103). `admin_close_stale_epoch` becomes the permanent write-off valve: its `migration_active` gate is removed, and it now requires the epoch to have ended and be undistributed. **And `distribute_epoch` short-circuits when `observations_submitted == 0`** — it pays nothing, touches no `Gateway.stats`, no accumulator and no treasury, marks the epoch distributed so the chain advances, and emits `EpochSkippedNoObservationsEvent` before the unchanged `EpochDistributedEvent`. |
| **Why (supersession)** | `weights_epoch` and `GatewaySlot.composite_weight` are per-tally, not per-epoch. While epoch N is unfinished anyone could `create_epoch(N+1)` and `tally_weights(N+1)`, re-stamping every gateway in N's undistributed range so N could never be paid correctly again. Mainnet epoch **540** is that loss: it stalled, 541 was created and tallied over it, and its remaining rewards were written off on 2026-09-15. 543 later stalled 3h20m in the same shape and survived only because every client in play happened to wait. Protection sat in client convention, in a system whose premise is that these instructions are permissionless. See ADR-0034. |
| **Why (the unobserved-epoch divergence)** | Distribution does not only pay — it also writes the record the incentive protocol runs on. Because a gateway is only ever marked failed when `observations_submitted > 0`, Lua's rule credits a pass to every gateway during an outage: it **wipes the failure streak** of any gateway heading for the 30-consecutive-failure prune, and **lifts the epoch pass rate** that gates ArNS operator-discount eligibility at 90%. An outage would launder the record of exactly the gateways the protocol exists to catch, on top of paying them. Skipping costs honest operators that period's rewards — the tokens stay in the treasury and fund later epochs — and that is the lesser harm. Consistent with ADR-0032/0033 and BD-115/BD-116, which already accept that a period which cannot be paid correctly simply goes unpaid. |
| **Deliberate consequence** | A **live** epoch in which every prescribed observer fails to submit also pays nobody and credits nobody. That is the same evidence vacuum as a catch-up epoch, and the alternative — paying blind — is worse. A stuck epoch now visibly halts the chain until it is distributed or written off, which makes the epoch authority's availability part of liveness (one more reason to move it to a multisig, ADR-0031). The skip carries a `distribution_index == 0` guard so an epoch already part-paid under the pre-Wave-2 program finishes normally instead of being recorded as a clean skip. |
| **Consumer note** | Do **not** infer a skip from `EpochDistributedEvent`'s zero totals — a normal distribution with an empty eligible set reports identically. `EpochSkippedNoObservationsEvent` is the only stable discriminator (ADR-018 froze the old event's shape). Anything that only needs "this epoch finished" keeps watching `EpochDistributedEvent` and needs no change. |

---

### BD-119: Registry Positions Are Frozen While an Epoch Is Unfinished (2026-09-18)

| | |
|---|---|
| **Lua Behavior** | Gateways live in an associative table keyed by address; observations and reward eligibility are likewise address-keyed (`epochs.lua` indexes `failureSummaries[gatewayAddress]` and `rewards.eligible[gatewayAddress]`). There are no positional slots, so removing a gateway mid-epoch cannot re-attribute another gateway's results. |
| **Solana Behavior (before)** | `GatewayRegistry` is a fixed-slot array and every epoch step addresses gateways **by position**: `tally_weights` writes `gateways[i].composite_weight`, `prescribe_epoch` builds prefix sums over positions, `save_observations` sets bit `i` into `epoch.failure_counts[i]`, and `distribute_epoch` pays `gateways[i]` judged by `failure_counts[i]`. `finalize_gone` — permissionless — swap-removes a slot, moving the LAST gateway into the freed index. Nothing checked epoch state, and `save_observations`' only alignment check is a count, not an ordering. |
| **Solana Behavior (now, ADR-0036)** | `finalize_gone` requires the latest Epoch PDA in `remaining_accounts` and refuses with `LatestEpochUnfinished` (6102) unless it is distributed or absent — the same predicate `create_epoch` uses, implemented once. Garbage collection therefore runs only between a completed distribution and the next epoch's creation. |
| **Why** | Mainnet epoch **542**: 14 `finalize_gone` calls from six distinct fee payers landed mid-window, after all 11 observations had been submitted against the 647-slot snapshot but before distribution ran. 14 gateways were judged at a different position than their observations were recorded against. `lasaucisse` (20,327 ARIO staked) was **failed by all 11 observers** at snapshot position 643, but distributed at position 149 where the departed gateway's count was 2 — so it was scored as **passed**, paid, and had its consecutive-failure counter reset rather than incremented, delaying its pruning. Epoch 543 repeated the pattern with 13 more removals. This is not rare: 27 removals across two consecutive epochs, only one of them from the reference cranker. A client guard cannot fix it — the SDK can refuse to *submit* a misaligned bitmap, but nothing client-side stops `distribute_epoch` reading a moved slot. See ADR-0036. |
| **Depends on BD-118** | The predicate inspects only the **latest** epoch, which is sufficient only because ADR-0034 guarantees no older epoch can still be unfinished. **ADR-0036 must never ship without ADR-0034.** |
| **Deliberate consequence** | Slot and rent reclamation now happen only in the gap between distribution and the next `create_epoch`. A cranker makes its sweep race-free by bundling `finalize_gone` into the same transaction as the final `distribute_epoch` batch; a sweep that misses the window succeeds in the next one. A departed operator cannot re-join until their slot is reclaimed, because `join_network` `init`s the Gateway PDA that `finalize_gone` closes. Registry capacity is 3,000 slots against ~620 in use, so starvation is not a payout risk; if it is observed in practice, ADR-0036's option 3 (tombstone, compact later) is the escape hatch. Third-party callers sweeping outside the window now waste a fee. |

---

### BD-120: A Gateway's Delegated-Stake Counter Must Equal Its Delegations (2026-09-18)

| | |
|---|---|
| **Lua Behavior** | A gateway's delegates live in `gateway.delegates`, a table keyed by delegator address, and `totalDelegatedStake` is maintained alongside it by the same handlers. There is no separate per-delegation account that can go missing, and no import step that could populate the total from one source and the members from another. |
| **Solana Behavior (before)** | `Gateway.total_delegated_stake` is a counter and each delegation is its own PDA, so the two are only equal if every writer maintains both. Two defects broke that. **(a)** The AO import wrote each counter from AO's total but created `Delegation` accounts only for delegators who had a Solana address; the rest of the stake went to the migration authority's pot and never entered the stake pool. **(b)** `settle_delegate_rewards` raises the gateway counter, but no caller raised `GatewaySettings.total_delegated` — `compound_delegation_rewards` had no `settings` account at all. Separately, `pay_from_funding_plan`'s sub-min residue left the supply counter high by the residue on every auto-vaulted payment. |
| **Solana Behavior (now, ADR-0037)** | `settle_delegate_rewards` returns the settled amount and is `#[must_use]`, so the compiler forces all 11 call sites to credit `settings.total_delegated`; `CompoundDelegationRewards` gains `settings` as its **last** account (trailing, so a client that appends it works against both the old and new program). The residue path subtracts its residue too. Two authority-only instructions are added: `admin_reconcile_delegated_stake(expected_counter, expected_removed)` lowers a gateway's counter to the sum of the `Delegation` accounts passed in `remaining_accounts`, and `admin_resync_supply_counters` sets the reporting-only supply counters to audited values. Errors 6104-6108. |
| **Why** | `counter − Σ Delegation.amount` is invariant under every other instruction, so an over-counted gateway can never reach zero on its own. On mainnet that is **2,226,210.675676 ARIO** across 132 gateways, reproduced to the mARIO from the genesis snapshot for 622 of 622 gateways. Consequences: `finalize_gone` is permanently blocked on 65 leaving gateways (23 with no `Delegation` accounts at all), re-enabling delegation is impossible for any over-counted gateway, 67 joined gateways carry stake weight and observer-selection odds for stake they do not have, and each epoch a delegate share is carved out and divided by the inflated counter — about 17,221 ARIO missing from real delegates so far, growing every epoch. The settlement defect separately drifted `settings.total_delegated` 80,442.894868 ARIO low. See ADR-0037. |
| **Safety model** | The program cannot enumerate a gateway's delegations, so completeness comes from **two independent sources having to agree**: the `Delegation` list from `getProgramAccounts`, and `expected_removed` from the genesis snapshot. Omit a delegation and the check fails unless both sources are wrong by the same amount — which is why `expected_removed` must NOT be derived from the same read. The counter can only be lowered, never raised, and `expected_counter` rejects a stale plan. **Residual risk:** drive a counter to 0 while real delegations exist and `finalize_gone` will close the Gateway PDA that `claim_delegate_from_leaving_gateway` needs, stranding that stake — so only run entries the audit marks `verified`, and treat `scripts/delegated-stake-audit.mjs`' under-count check (exit status 1) as a required post-step. |
